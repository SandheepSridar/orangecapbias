"""TNPL mirror of src/stats.py — statistical tests for the Most Runs award bias.

Tests run (same battery as the IPL study):
  A. Chi-square: winner position distribution vs overall batter population
  B. Kruskal-Wallis + Mann-Whitney post-hoc: balls faced across position groups
...
Thresholds rescaled for TNPL (league = 7 matches): qualification 4+ matches,
normalisation to the season's league length (vs the IPL's 14).

Results saved to TNPL/tables/stats_results.csv
"""
...
OUT = ROOT / "tables" / "stats_results.csv"
...
winners = pd.read_csv(ROOT / "tables" / "analysis_a_winner_positions.csv")
...
pop = batter_season[batter_season["matches"] >= MIN_MATCHES]
...
print("  Post-hoc (Opener vs Middle Order):")
...
league_bs = league_bs[league_bs["league_matches"] >= MIN_MATCHES]
...
# Normalisation baseline = the season's league length (TNPL: 7), mirroring the
# IPL pipeline's 14. Playoff players are scaled only for the league length;
# non-playoff players are projected up to a full league season.
league_len = league_bs.groupby("season")["league_matches"].transform("max")
denom = np.where(league_bs["made_playoffs"] == 1, league_len, league_bs["league_matches"])
league_bs["norm_runs"] = league_bs["league_runs"] * league_len / denom
...
missing = (league_len - league_bs["league_matches"]).clip(lower=0)
...
d_out[["season", "batter", "league_matches", "league_runs", "normalised_runs",
    ...
    ["season", "actual_rank"]
).to_csv(ROOT / "tables" / "analysis_d_normalised_rankings.csv", index=False)
...
full_bs = full_bs[full_bs["matches"] >= MIN_MATCHES]
